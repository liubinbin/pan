...
public class BinTreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, Serializable {
	/**
	 ...
	 * given comparator: {@code comparator.compare(k1,
	 * k2)} must not throw a {@code ClassCastException} for any keys {@code k1} and
	 * {@code k2} in the map. If the user attempts to put a key into the map that
	 * violates this constraint, the {@code put(Object
	 ...
	 *            keys will be used.
	 */
	public BinTreeMap(Comparator<? super K> comparator) {
	 ...
	 */
	public BinTreeMap(Map<? extends K, ? extends V> m) {
		comparator = null;
		putAll(m);
	}
	 ...
	 *             if the specified map is null
	 */
	public BinTreeMap(SortedMap<K, ? extends V> m) {
	 ...
	 * @throws NullPointerException
	 *             if the specified map is null or the specified map contains a null
	...
	public V put(K key, V value) {
		...
		if (cmp < 0)
			...
			parent.right = e;
		System.out.println("do fixAfterInsertion");
		fixAfterInsertion(e);
		size++;
		modCount++;
		return null;
	...
	/**
	 * Returns a shallow copy of this {@code TreeMap} instance. (The keys and values
	 ...
	 * {@link Spliterator#getComparator()}) is {@code null} if the tree
...
//	}
	 ...
	 *             {@inheritDoc}
	 * @throws NullPointerException
	 *             if {@code toKey} is null and this map uses natural ordering, or
	 ...
	 *             {@inheritDoc}
	 * @throws NullPointerException
	 *             if {@code fromKey} is null and this map uses natural ordering, or
	 *             its comparator does not permit null keys
	 * @throws IllegalArgumentException
	 *             {@inheritDoc}
	 * @since 1.6
	...
	class Values extends AbstractCollection<V> {
		...
		public int size() {
			return BinTreeMap.this.size();
		}

		public boolean contains(Object o) {
			return BinTreeMap.this.containsValue(o);
		}
		...
		public void clear() {
			BinTreeMap.this.clear();
		}

		public Spliterator<V> spliterator() {
			return new ValueSpliterator<K, V>(BinTreeMap.this, null, null, 0, -1, 0);
		}
	}
	...
	class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public Spliterator<Map.Entry<K, V>> spliterator() {
			return new EntrySpliterator<K, V>(BinTreeMap.this, null, null, 0, -1, 0);
		}
	}
	...
	static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
		...
		public Iterator<E> iterator() {
			if (m instanceof BinTreeMap)
				return ((BinTreeMap<E, ?>) m).keyIterator();
			else
		...
		public Iterator<E> descendingIterator() {
			if (m instanceof BinTreeMap)
				return ((BinTreeMap<E, ?>) m).descendingKeyIterator();
			else
	...
	abstract static class NavigableSubMap<K, V> extends AbstractMap<K, V>
		...
		NavigableSubMap(BinTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		final BinTreeMap.Entry<K, V> absLowest() {
			BinTreeMap.Entry<K, V> e = (fromStart ? m.getFirstEntry()
					: (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final BinTreeMap.Entry<K, V> absHighest() {
			BinTreeMap.Entry<K, V> e = (toEnd ? m.getLastEntry()
					: (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final BinTreeMap.Entry<K, V> absCeiling(K key) {
			if (tooLow(key))
				return absLowest();
			BinTreeMap.Entry<K, V> e = m.getCeilingEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final BinTreeMap.Entry<K, V> absHigher(K key) {
			if (tooLow(key))
				return absLowest();
			BinTreeMap.Entry<K, V> e = m.getHigherEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final BinTreeMap.Entry<K, V> absFloor(K key) {
			if (tooHigh(key))
				return absHighest();
			BinTreeMap.Entry<K, V> e = m.getFloorEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final BinTreeMap.Entry<K, V> absLower(K key) {
			if (tooHigh(key))
				return absHighest();
			BinTreeMap.Entry<K, V> e = m.getLowerEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		/** Returns the absolute high fence for ascending traversal */
		final BinTreeMap.Entry<K, V> absHighFence() {
			return (toEnd ? null : (hiInclusive ? m.getHigherEntry(hi) : m.getCeilingEntry(hi)));
		}

		/** Return the absolute low fence for descending traversal */
		final BinTreeMap.Entry<K, V> absLowFence() {
		...
		abstract BinTreeMap.Entry<K, V> subLowest();

		abstract BinTreeMap.Entry<K, V> subHighest();

		abstract BinTreeMap.Entry<K, V> subCeiling(K key);

		abstract BinTreeMap.Entry<K, V> subHigher(K key);

		abstract BinTreeMap.Entry<K, V> subFloor(K key);

		abstract BinTreeMap.Entry<K, V> subLower(K key);
		...
		public final Entry<K, V> pollFirstEntry() {
			BinTreeMap.Entry<K, V> e = subLowest();
		...
		public final Entry<K, V> pollLastEntry() {
			BinTreeMap.Entry<K, V> e = subHighest();
			Entry<K, V> result = exportEntry(e);
			if (e != null)
				m.deleteEntry(e);
			return result;
		...
		abstract class EntrySetView extends AbstractSet<Entry<K, V>> {
			...
			public boolean isEmpty() {
				BinTreeMap.Entry<K, V> n = absLowest();
			...
			public boolean contains(Object o) {
				...
				if (!inRange(key))
					return false;
				BinTreeMap.Entry<?, ?> node = m.getEntry(key);
			...
			public boolean remove(Object o) {
				...
				if (!inRange(key))
					return false;
				BinTreeMap.Entry<K, V> node = m.getEntry(key);
		...
		abstract class SubMapIterator<T> implements Iterator<T> {
			BinTreeMap.Entry<K, V> lastReturned;
			BinTreeMap.Entry<K, V> next;
			final Object fenceKey;
			int expectedModCount;

			SubMapIterator(BinTreeMap.Entry<K, V> first, BinTreeMap.Entry<K, V> fence) {
			...
			final BinTreeMap.Entry<K, V> nextEntry() {
				BinTreeMap.Entry<K, V> e = next;
			...
			final BinTreeMap.Entry<K, V> prevEntry() {
				BinTreeMap.Entry<K, V> e = next;
				if (e == null || e.key == fenceKey)
					throw new NoSuchElementException();
				if (m.modCount != expectedModCount)
		...
		final class SubMapEntryIterator extends SubMapIterator<Entry<K, V>> {
			SubMapEntryIterator(BinTreeMap.Entry<K, V> first, BinTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapEntryIterator extends SubMapIterator<Entry<K, V>> {
			DescendingSubMapEntryIterator(BinTreeMap.Entry<K, V> last, BinTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
		...
		final class SubMapKeyIterator extends SubMapIterator<K> implements Spliterator<K> {
			SubMapKeyIterator(BinTreeMap.Entry<K, V> first, BinTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapKeyIterator extends SubMapIterator<K> implements Spliterator<K> {
			DescendingSubMapKeyIterator(BinTreeMap.Entry<K, V> last, BinTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
	...
	static final class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		AscendingSubMap(BinTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
		...
		BinTreeMap.Entry<K, V> subLowest() {
			return absLowest();
		}

		BinTreeMap.Entry<K, V> subHighest() {
			return absHighest();
		}

		BinTreeMap.Entry<K, V> subCeiling(K key) {
			return absCeiling(key);
		}

		BinTreeMap.Entry<K, V> subHigher(K key) {
			return absHigher(key);
		}

		BinTreeMap.Entry<K, V> subFloor(K key) {
			return absFloor(key);
		}

		BinTreeMap.Entry<K, V> subLower(K key) {
			return absLower(key);
		}
	}
	...
	static final class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		DescendingSubMap(BinTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
			super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
		}
	 ...
	 * previous releases of TreeMap that did not support NavigableMap. It translates
	 * an old-version SubMap into a new-version AscendingSubMap. This class is never
	 * otherwise used.
	 *
	 * @serial include
	 */
	private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>, Serializable {
		...
		private Object readResolve() {
			return new AscendingSubMap<>(BinTreeMap.this, fromStart, fromKey, true, toEnd, toKey, false);
		}
	...
	/**
	 * Save the state of the {@code TreeMap} instance to a stream (i.e., serialize
	 * it).
	 *
	...
	/**
	 * Reconstitute the {@code TreeMap} instance from a stream (i.e., deserialize
	 * it).
	 */
	...
	/**
	 * Linear time tree building algorithm from sorted data. Can accept keys and/or
	 * values from iterator or stream. This leads to too many parameters, but seems
	 * better than alternatives. The four formats that this method accepts are:
	 *
	 * 1) An iterator of Map.Entries. (it != null, defaultVal == null). 2) An
	 * iterator of keys. (it != null, defaultVal != null). 3) A stream of
	 * alternating serialized keys and values. (it == null, defaultVal == null). 4)
	 ...
	 *            from this iterator.
	 * @param str
	 *            If non-null, new entries are created from keys and possibly values
	 *            read from this stream in serialized form. Exactly one of it and
	 *            str should be non-null.
	 * @param defaultVal
	 *            if non-null, this default value is used for each value in the map.
	 *            If null, each value is read from iterator or stream, as described
	 *            above.
	 ...
	 * fields of the TreeMap are already set prior to calling this method. (It
	 * ignores both fields.)
	 *
	 * @param level
	...
	private final Entry<K, V> buildFromSorted(int level, int lo, int hi, int redLevel, Iterator<?> it,
		...
		/*
		 * Strategy: The root is the middlemost element. To get to it, we have to first
		 * recursively construct the entire left subtree, so as to grab all of its
		 * elements. We can then proceed with right subtree.
		 *
		 * The lo and hi arguments are the minimum and maximum indices to pull out of
		 * the iterator or stream for current subtree. They are not actually indexed, we
		 * just proceed sequentially, ensuring that items are extracted in corresponding
		 * order.
		 */
	 ...
	 * `full' level of the complete binary tree produced by buildTree. The remaining
	 * nodes are colored RED. (This makes a `nice' set of color assignments wrt
	 * future insertions.) This level number is computed by finding the number of
	 * splits needed to reach the zeroeth node. (The answer is ~lg(N), but in any
	...
	static <K> Spliterator<K> keySpliteratorFor(NavigableMap<K, ?> m) {
		if (m instanceof BinTreeMap) {
			@SuppressWarnings("unchecked")
			BinTreeMap<K, Object> t = (BinTreeMap<K, Object>) m;
			return t.keySpliterator();
		}
		if (m instanceof DescendingSubMap) {
			@SuppressWarnings("unchecked")
			DescendingSubMap<K, ?> dm = (DescendingSubMap<K, ?>) m;
			BinTreeMap<K, ?> tm = dm.m;
			if (dm == tm.descendingMap) {
				@SuppressWarnings("unchecked")
				BinTreeMap<K, Object> t = (BinTreeMap<K, Object>) tm;
	 ...
	 * ascending split rules. This base class is non-commital about directionality,
	 ...
	 * default mechanics.
	 *
	 * To boostrap initialization, external constructors use negative size
	 * estimates: -1 for ascend, -2 for descend.
	 */
	static class TreeMapSpliterator<K, V> {
		final BinTreeMap<K, V> tree;
		...
		TreeMapSpliterator(BinTreeMap<K, V> tree, Entry<K, V> origin, Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			this.tree = tree;
			this.current = origin;
			this.fence = fence;
	...
	static final class KeySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<K> {
		KeySpliterator(BinTreeMap<K, V> tree, Entry<K, V> origin, Entry<K, V> fence, int side, int est,
				int expectedModCount) {
	...
	static final class DescendingKeySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<K> {
		DescendingKeySpliterator(BinTreeMap<K, V> tree, Entry<K, V> origin, Entry<K, V> fence, int side,
	...
	static final class ValueSpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<V> {
		ValueSpliterator(BinTreeMap<K, V> tree, Entry<K, V> origin, Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			super(tree, origin, fence, side, est, expectedModCount);
		}
	...
	static final class EntrySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<Map.Entry<K, V>> {
		EntrySpliterator(BinTreeMap<K, V> tree, Entry<K, V> origin, Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			super(tree, origin, fence, side, est, expectedModCount);
		}
